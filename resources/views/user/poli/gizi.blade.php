        ...
        <div class="row g-5">
            <div class="col-lg-6 wow fadeIn" data-wow-delay="0.1s">
                <div class="d-flex flex-column">
                    ...
                    <p style="color: black;">Konseling gizi adalah interaksi dua arah di mana pasien dan anggota tim medis menafsirkan hasil penilaian gizi,
                        mengidentifikasi masalah gizi pasien, kebutuhan dan tujuan, mendiskusikan cara untuk mencapai tujuan ini, dan menyepakati langkah-langkah masa depan dan frekuensi pemantauan.
                    </p>
                </div>
            </div>
            <div class="col-lg-6 wow fadeIn" data-wow-delay="0.5s">
                ...
                {{-- {{ dd($konsul) }} --}}
                @if ($konsul->exists)
                <form action="{{ url('konsultasi_umum/edit/'.$konsul->id) }}" method="POST">
                    @else
                    <form action="{{ url('konsultasi_umum') }}" method="POST">
                        ...
                        <div class="mb-3 mt-3">
                            ...
                            <input type="text" class="form-control @error('judul') is-invalid @enderror" name="judul" id="judul" placeholder="Masukkan judul keluhan Anda" value="{{ old('judul')}}" required autofocus>
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3 mt-3">
                            ...
                            <input type="number" min="1" class="form-control" placeholder="Masukkan usia" name="usia" value="{{ old('usia', $konsul->usia) }}" required autofocus>
                        </div>
                        <div class="mb-3 mt-3">
                            <label for="email" class="form-label">Berat Badan (Kg)* :</label>
                            <input type="number" min="1" class="form-control" placeholder="Masukkan berat badan anda" name="berat_badan" value="{{ old('berat_badan', $konsul->berat_badan) }}" required autofocus>
                        </div>
                        <div class="mb-3 mt-3">
                            <label for="email" class="form-label">Tinggi Badan (cm) :</label>
                            <input type="number" min="1" class="form-control" placeholder="Masukkan Tinggi anda" name="tinggi_badan" value="{{ old('tinggi_badan', $konsul->tinggi_badan) }}" required autofocus>
                        </div>
                        <div class="mb-3 mt-3">
                            <label for="email" class="form-label">Golongan Darah:</label>
                            <input type="text" class="form-control" placeholder="Masukkan golongan darah anda" name="gol_darah" value="{{ old('gol_darah', $konsul->gol_darah) }}" required autofocus>
                        </div>
                        <div class=" mb-3">
                            <label for="comment">Keluhan*:</label>
                            <textarea class="form-control" rows="5" id="comment" name="keluhan" placeholder="ketikkan keluhan anda" required autofocus>{{ old('keluhan', $konsul->keluhan) }}</textarea>
                        </div>
                        <center>
